...
//
//
static const GUID COOLMENU_GUID = {
	0x44de039e, 0x73ac, 0x4025, {
		0xa3, 0x3f, 0x99, 0x7, 0xc, 0x4e, 0x40, 0x23 	}
};
...
static BOOL CoolMenu_PrintLable(HDC hdc, HFONT hFont, RECT rc, LPCTSTR lpszLable)
{
	...
	ok = TextOut(hdc
		    , CXMENUADJUST
		    , rc.bottom - CXMENUADJUST - CXMENUADJUST
		    , lpszLable
		    , lstrlen(lpszLable));
...
//
BOOL CoolMenu_Create(HMENU hmenu, LPCTSTR lpVertLable)
{
	...
	CoolMenuItemData *cmid = HeapAlloc(GetProcessHeap()
		    , HEAP_ZERO_MEMORY, sizeof(CoolMenuItemData) * menucount);
					...
					if( tabptr != 0 )
					{
						...
						if( bSelected || !bGrayed )
						{
							...
							DrawTextEx(dis->hDC
							    ...
							    , -1
							    , &rect
							    , DT_RIGHT | DT_SINGLELINE | DT_VCENTER | DT_EXPANDTABS
							    , NULL);
